.. raw:: html

     <div id="leftcolumn" class="edoBox">
   <dl class="portlet portlet-navigation-tree">
    <dt class="portletHeader">
    <a class="title" href="http://everydo.com/purchase/">产品购买</a>
    </dt>
    <dd class="portletItem">

    <li class="navTreeItem visualNoMarker node"
       kssattr:nodeurl="http://everydo.com/purchase/index.rst">
       <div class="visualIcon contenttype-folder">

.. navtree::
   :root_depth: 1
   :class: portletNavigationTree navTreeLevel0

.. raw:: html

   </div>
   </li>
   </ul>
   </dd>
   <dd class="portletFooter">
   </dd>
    <div class="edoBox">
        <a href="http://oc.easydo.cn/@@signup.html?setup=zopen.setups.all"><img  src="img/register.png" alt="注册" style="margin-left:12px"></a>
    </div>
    <p style="text-align:center; height:30px; margin-top:20px; margin-bottom:10px;">
        <img src="img/pic5.gif"/><span></span> 
        <a href="http://demo.oc.easydo.cn/@@loginForm.html?camefrom=%2Flogin%3Fservice%3Dhttp%253A%252F%252Fdemo.easydo.cn&isdocsdemo=1" style="font-size:14px;"><b>立即进入体验站点</b></a>
    </p>

    </div>

